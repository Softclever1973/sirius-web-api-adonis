export async function sendPasswordResetEmail(email, nome, token, baseUrl) {
    ...
    const EMAIL_API_URL = process.env.EMAIL_API_URL || 'http://localhost:3001';

    const response = await fetch(`${EMAIL_API_URL}/email/enviar`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'x-api-key': process.env.EMAIL_API_SECRET_KEY,
        },
        body: JSON.stringify({
            to: email,
    ...
    if (!response.ok || !data.success) {
        throw new Error(`Falha ao enviar e-mail: ${data.message || response.statusText}`);
    }
}

// =====================================================
// IMPLEMENTAÇÃO ANTERIOR (Nodemailer via SMTP)
// Bloqueado pelo Vercel — manter para uso local/futuro
// =====================================================
